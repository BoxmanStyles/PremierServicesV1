...
public class AdminPaginaPrincipalController {
    ...
    // Abrir el panel de administración (AdminPanel.fxml)
    @FXML
    private void AbrirAdminPanel(ActionEvent event) {
        try {
            String fxmlPath = "/AdminPanel.fxml";
            URL url = getClass().getResource(fxmlPath);
            if (url == null) {
                url = getClass().getResource("/com/example/premierservices/AdminPanel.fxml");
            }
            if (url == null) {
                mostrarAlerta("Error", "No se encontró AdminPanel.fxml");
                return;
            }
            ...
            e.printStackTrace();
            mostrarAlerta("Error", "No se pudo abrir el panel de administración: " + e.getMessage());
        }
    }

    // Cerrar sesión (volver al login general)
    @FXML
